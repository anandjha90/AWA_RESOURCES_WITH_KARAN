...
CREATE DATABASE IF NOT EXISTS CLASS_7_DATABASE1;

-- CREATING a schema for the SALES_DATABASE 
CREATE SCHEMA IF NOT EXISTS CLASS_7_SCHEMA1;

-- USING THE DATABASE CREATED SALES_DATABASE 
USE DATABASE CLASS_7_DATABASE1;

-- USING THE SCHEMA CREATED SALES_SCHEMA
USE SCHEMA CLASS_7_SCHEMA1;
...
SELECT CURRENT_TIMESTAMP();
SELECT 
    TO_CHAR(TIME(CURRENT_TIMESTAMP()), 'MM:HH:SS:FF'),
    TO_CHAR(TIME(CURRENT_TIMESTAMP()), 'MM:HH');

-- 
SELECT
    YEAR(CURRENT_TIMESTAMP()),
    YEAR(DATE(CURRENT_TIMESTAMP())),
    YEAR(TO_DATE('2025-02-02'));
...
SELECT 
    ...
    MINUTE(CURRENT_TIMESTAMP()) AS MIN_FROM_DT,
    SECOND(CURRENT_TIMESTAMP()) AS SEC_FROM_DT;
...
SELECT
    ...
    DELIVERY_DATE,
    RETURN_DATE,
    DATEDIFF(DAYS, TRANSACTION_DATE, DELIVERY_DATE) AS DELIVERY_TIME,
    DATEDIFF(MONTHS, TRANSACTION_DATE, RETURN_DATE) AS RETURN_AFTER,
    DATEDIFF(YEARS, TRANSACTION_DATE, RETURN_DATE) AS RETURN_AFTER_YEARS
FROM SALES_TRANSACTIONS;
...
SELECT
    ...
    DATEADD(MONTHS, 1, TRANSACTION_DATE) AS EXPECTED_DELIVERY_DATE_OLD
FROM SALES_TRANSACTIONS
...
SELECT
    TRANSACTION_DATE,
    DATEADD(MONTHS, 4, TRANSACTION_DATE) AS CORRECT_TRANSACTION_DATE,
    ADD_MONTHS(TRANSACTION_DATE, 4) AS CORRECT_TRANSACTION_DATE_NEW
FROM SALES_TRANSACTIONS;
...
SELECT
    ...
    MONTHS_BETWEEN(TO_DATE('2025-01-01'), TO_DATE('2025-02-01')) AS DEF_4
FROM SALES_TRANSACTIONS;


SELECT
    TO_DATE('2025-01-20') AS START_DATE,
    TO_DATE('2025-02-01') AS END_DATE,
    MONTHS_BETWEEN(TO_DATE('2025-03-20'), TO_DATE('2025-02-20')) AS DEF_4;